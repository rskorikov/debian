...
wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- | sudo tee /etc/apt/keyrings/packages.mozilla.org.asc > /dev/null -y
# 4. Add the Mozilla signing key to your sources.list
echo "deb [signed-by=/etc/apt/keyrings/packages.mozilla.org.asc] https://packages.mozilla.org/apt mozilla main" | sudo tee -a /etc/apt/sources.list.d/mozilla.list > /dev/null -y
...
Pin-Priority: -1' | sudo tee /etc/apt/preferences.d/mozilla -y
...
sudo apt install firefox firefox-l10n-ru -y
